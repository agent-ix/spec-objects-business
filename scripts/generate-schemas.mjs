 ...
 * `$ref` the emitter left relative, writes `spec_objects_business/schemas/`
...
const SEMANTIC_CORE_BASE = "https://schemas.agent-ix.org/semantic-core/0.2.0/";
const NORMALIZATION = {
  name: "absolute-id-and-ref",
  version: "1.0.0",
  issue: "https://github.com/agent-ix/spec-objects-business/issues/4",
};

const repoRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..");
const sourceDir = resolve(repoRoot, "typespec");
const packageDir = resolve(repoRoot, "spec_objects_business");
...
function moduleBase() {
  ...
  const expected = `https://schemas.agent-ix.org/agent-ix/spec-objects-business/${version}/`;
...
function emit() {
  ...
  const scratch = mkdtempSync(join(tmpdir(), "spec-objects-business-emit-"));
  try {
    compile(scratch);
      ...
      normalization: {
        ...
        note:
          rewrittenFiles.length === 0
            ...
            : "rewrote relative $id/$ref to the module base or semantic-core 0.2.0",
      },
      files: [...rendered.keys()],
