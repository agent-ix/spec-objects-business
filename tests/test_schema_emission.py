...
def test_emitted_set_is_the_twenty_one_files_the_toolchain_records():
    record = toolchain()
    expected = sorted(
...
def test_schemas_check_is_green_on_the_committed_tree_and_names_a_mutation(tmp_path):
    ...
    manifest = tree / "spec_objects_business" / "manifest.yaml"
    manifest.write_text(
        re.sub(
            r"digest: sha256:\w+",
...
def test_a_coordinated_version_bump_reemits_every_id_and_digest(tmp_path):
    ...
    assert run_generator(cwd=tree).returncode == 0
    bumped_base = f"https://schemas.agent-ix.org/agent-ix/spec-objects-business/{new}/"
...
def test_no_test_hard_codes_the_id_version_segment():
    ...
    literal = f"spec-objects-business/{version}/"
