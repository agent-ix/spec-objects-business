...
- Every construct `meaning` SHALL be a QSpec FR-208 meaning id, spelled exactly as FR-208 lists it at `c8e3ca0`. FR-208 gives each kind's invariants (`## Invariants` clauses), `Pre:`/`Post:` operation contracts and `specializes` generalization their meaning, so the module refuses none of them on `value_object`, `event`, `state_machine` or `process`. The refusals FR-208 itself states stand at intake: an operation on a record value type or an event type, and supertypes or clauses on a persistence interface or a namespace.
