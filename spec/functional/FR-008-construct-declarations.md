...
- A `construct:` on each of the ten object types in `manifest.yaml`; none on
  `population`.
...
- `population` SHALL declare no construct.
...
|----|----------|--------------|
| FR-008-AC-1 | Exactly the ten construct kinds declare a `construct:`, `population` declares none, and the manifest has zero violations against the FR-035 schema at `e33070e`. | Test |
...
- **Upstream**: filament-core-service FR-035 CR-004 at `e33070e`; filament-core-data FR-142 (`agent-ix/filament-core-data#172`); QSpec FR-208; [FR-003](./FR-003-semantic-manifest-contract.md)
- **Downstream**: `agent-ix/quire-rs#445` carries each declaration on `CompiledArchetype::construct()`; filament-core-data pins this manifest version
