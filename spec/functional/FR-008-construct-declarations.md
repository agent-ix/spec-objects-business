...
- The module SHALL refuse every form FR-208 (`c8e3ca0`) states as an intake refusal for the meaning a kind binds, wherever this module's own record schema (FR-004) governs the refused member:
  - `ValueObject.json` (`record-value-type/v1`) carries no `operations` key: an operation on a `value_object` is refused (`unsupported_construct`/`declaration-form`).
  - `Event.json` (`event-type/v1`) carries no `operations` key: an operation on an `event` is refused (`invalid_model_binding`/`malformed-declaration`).
  - `Repository.json` (`persistence-interface/v1`) carries no `clauses` key, and each `operations[]` entry's `pre` and `post` must be empty or absent: a `quire` clause, and a `Pre:`/`Post:` contract, on a `repository` operation are each refused (`unsupported_construct`/`declaration-form`). FR-208 also refuses a non-empty operation frame (`Modifies:`/`Creates:`/`Deletes:`) on a persistence interface; that data reaches no member this module's record schema declares (FR-006, blocked on `agent-ix/quire-rs#431`), so this module cannot refuse it at the record-schema layer.
  - `Domain.json` (`namespace/v1`) carries no `fields`, `operations`, or `clauses` key: a field, an operation, or a `quire` clause on a `domain` is refused (`invalid_model_binding`/`malformed-declaration` for a field or operation; `unsupported_construct`/`declaration-form` for a clause).
