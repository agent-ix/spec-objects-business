...
- Exactly seven skeletons (`entity`, `nested_entity`, `value_object`, `aggregate_root`, `event`, `state_machine`, `process`) SHALL author `## Invariants` with one `### <clauseId>` per clause, each owning exactly one ```` ```ocl ```` fence.
- The `domain`, `repository`, and `enumeration` skeletons SHALL carry no `## Invariants` section.
...
- Each skeleton whose type requires `operations` (`repository`, `state_machine`) SHALL author `## Operations` with one `### <name>` per operation, an optional `| Param | Type | Multiplicity | Constraints |` table, a `Returns:` line where the operation returns a value, and optional `Pre:`/`Post:` lines that, when present, name clause ids declared in the same artifact (the `repository` skeleton declares no clauses and so writes none).
...
- The `enumeration`, `domain`, `event`, `state_machine`, `process`, `aggregate_root`, and `nested_entity` skeletons SHALL keep their kernel sections (`## Values` with `Value | Description`, `## Bounded Context`, the `## Schema` JSON fence, `## States & Transitions`, `## Workflow`, `## Members`, `## Parent`) with the heading, table columns, and fence language the manifest asserts.
- Where a typed section and a kernel section describe the same declarations (the event `## Schema` fence and its `## Properties` table; the state-machine diagram and its `## Operations`; `## Members`, `## Parent`, `## Workflow` prose and the optional `members`, `owner`, `steps` keys), the typed section SHALL be the authority and the kernel section a derived, human-facing view; the enumeration `## Values` table stays the asserted form while `agent-ix/quoin#335` decides the `EnumValue[]` mapping.
