...
0.2.0, using the official `@typespec/json-schema` emitter at a pinned
...
- `@agent-ix/semantic-core` 0.2.0 from npm.ix (`FieldDecl`, `TypeRef`,
...
- `@typespec/compiler` 1.15.0, `@typespec/json-schema` 1.15.0 and
  `@agent-ix/semantic-core` 0.2.0 as exact `devDependencies` in `package.json`,
...
- If the emitter leaves any `$id` or `$ref` relative, then the generator SHALL rewrite it to `<base><file>` (module models) or `https://schemas.agent-ix.org/semantic-core/0.2.0/<file>` (semantic-core models) and record each rewrite in `toolchain.json`.
...
- In `--check` mode the generator SHALL write no file, neither under `spec_objects_business/schemas/` nor in `manifest.yaml`.
...
- Every `$ref` in an emitted schema SHALL name either a sibling `https://schemas.agent-ix.org/agent-ix/spec-objects-business/<manifest version>/<File>.json` that ships in `schemas/`, or `https://schemas.agent-ix.org/semantic-core/0.2.0/<Model>.json`.
...
- If any emitted file differs from the committed output, a committed file under `spec_objects_business/schemas/` is stale (it has no emitted counterpart in this run), `toolchain.json` differs, or a manifest digest differs from the shipped bytes, then the check SHALL exit non-zero naming each such file.
...
|----|----------|--------------|
| FR-002-AC-1 | After `make schemas`, `spec_objects_business/schemas/` holds exactly the twenty-one files `toolchain.json` lists (eleven object-type models plus the support models `IdentityField`, `OccurrenceField`, `OccurrenceTypeRef`, `Term`, `Transition`, `ProcessStep`, `StepKind`, `PopulationMember`, and the FR-009 `ObjectId` and `ObjectFrontmatter`), with compiler 1.15.0 and emitter 1.15.0 recorded. | Test |
...
| FR-002-AC-9 | `make schemas-check` on a committed tree carrying an extra `spec_objects_business/schemas/Stale.json` with no emitted counterpart exits non-zero naming that file, and writes nothing. | Test |
