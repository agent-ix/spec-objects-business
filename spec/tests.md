...
schemas and model tables (US-001, FR-002..FR-006, NFR-001, IT-002). Coverage is complete when
...
4. Every named refusal (digest mismatch, unknown key, both forms, dangling clause, non-Identifier token, undeclared model form, unknown state, trigger, or step kind) has a failing fixture.
...
| FR-006 | FR-006-AC-1..5, FR-006-CON-1 | TC-080..TC-084 | ✅ |

### Non-Functional Requirement Coverage

| Non-Functional Req | Verification Method | Evidence/Test Cases | Status |
|---|---|---|---|
| NFR-001 | Test (NFR-001-AC-1..4: locator baseline diff outside the FR-006 model tables, legacy skeleton validation) | TC-060..TC-063 | ✅ |
...
| TC-054 | Every negative fixture fails with its `expect:` code and the eighteen named cases exist | Integration | P0 | FR-005-AC-5 | ✅ |
...
| TC-080 | The model-table `table_row` locators are exactly the seven FR-006 declares, with their sections, column sets, `required` flags, and `min_rows: 1` | Unit | P0 | FR-006-AC-1, FR-006-CON-1 | ✅ |
...
| TC-082 | The declared-model fixture extracts supertypes, `abstract`, field presence, subsets, redefines, and one operation frame | Integration | P0 | FR-006-AC-3 | ✅ |
...
| TC-084 | Skeleton clauses are `quire` with no advisory; no skeleton carries `ocl`, `Pre:`/`Post:`, or a model-section diagram | Integration | P1 | FR-006-AC-5 | ✅ |
