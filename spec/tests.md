...
one test case. Rows are `🚧` until a tagged test asserts them; `quire coverage` reports 101/101 rows backed on this branch, and the rows still `🚧` are the ones whose evidence needs an environment this repository cannot provision (a running `filament-core-service`, a Quoin built from main).
